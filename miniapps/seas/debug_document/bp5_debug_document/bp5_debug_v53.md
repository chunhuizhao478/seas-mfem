...
### 16.11 Direct operator `b(slip)` test: passes to roundoff

To test the specific Section 16 hypothesis that `AssembleSlipContributionIP`
may be inconsistent with the DG stiffness matrix, we added a direct operator
test in
`miniapps/seas/tests/unit/test_elasticity_operator.cpp`:

- build a small `p=1` tet/IP problem
- construct a heterogeneous prescribed slip field
- solve for `u`
- assemble the operator's actual slip-only RHS `b(slip)` through the production
  path
- assemble the matching DG matrix `K`
- compare `K u` against `b(slip)`

Observed result:

- `||b_operator|| = 5.55386e+12`
- `||Au - b|| = 2.4014`
- relative mismatch `= 4.32384e-13`

So on the tested small `p=1` tet/IP problem, the **production slip RHS
assembly is algebraically consistent with `K`**. This weakens the earlier
Section 16 suspicion that the remaining mismatch is simply a broken
`AssembleSlipContributionIP` algebra bug.

### 16.12 Full node-level BP5 `ComputeRHS` vs Tandem source: passes

We next moved one layer up and compared the full BP5 constitutive RHS against
explicit Tandem-source formulas on **all fault nodes** under:

- nonzero heterogeneous traction
- nonzero imposed slip
- elastic normal-stress feedback

Test location:

- `miniapps/seas/tests/unit/test_bp5_fault_operator.cpp`
...
So the BP5 friction/state update path itself is matching the Tandem-source
formulas on this test.

### 16.13 Rejected-step purity test: no cache/state pollution

Because the remaining live differences had shifted to the dynamic side, we
tested whether rejected RK45/V-guard attempts leave side effects in the BP5/IP
operator that change the next accepted step.

Test setup:

- `miniapps/seas/tests/unit/test_bp5_fault_operator.cpp`
- working BP5/IP fixture (`DGMethod::IP`)
- initialize BP5 state, then apply a heterogeneous slip perturbation
- run RK45 with very large `dt=1e6` and `VGuard=1.05`
- let the solver reject repeatedly until the first accepted step
- record the accepted step size
- rerun from the same initial state on a fresh operator with that accepted
  reduced `dt`
- compare accepted state, traction, and `V_max`

Observed result:

- rejected attempts before first acceptance: `17`
- accepted reduced step: `dt = 1.52588 s`
- relative state difference: `0`
- relative traction difference: `0`
- relative `V_max` difference: `0`

Interpretation:

- rejected V-guard attempts are **not** polluting the subsequent accepted BP5/IP
  step
- the dynamic mismatch is **not** coming from stale cached displacement,
  traction, or slip-rate state surviving rejections

### 16.14 Updated narrowed conclusion

At this point we have ruled out, with explicit tests:

- local IP consistency/symmetry matrix mismatch
- local traction decomposition mismatch
- global slip RHS vs `K` inconsistency
- node-level BP5 `ComputeRHS` mismatch vs Tandem source
- rejected-step cache/state pollution in the BP5/IP RK path
- post-step psi clamp as the primary cause (A/B run did not help)

So the remaining live differences are now concentrated in the **driver-level
time-step policy itself**, not in rejected-step side effects:

1. MFEM's custom startup `dt_init`
2. MFEM's custom stage-level V-guard acceptance/rejection rule
3. more generally, the accepted step sequence generated by the custom RK45
   driver vs Tandem/PETSc TS adapt

This is weaker than a full Tandem executable-to-executable proof, but it is
the cleanest remaining source-level conclusion available from the local test
suite.
