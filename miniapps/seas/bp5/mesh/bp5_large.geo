// BP5-QD 3D benchmark mesh — LARGE DOMAIN for boundary effect testing
// Based on bp5.geo with 4x domain size to isolate boundary effects
//
...
//
// Usage:
//   gmsh -3 bp5_large.geo -o bp5_large_coarse.msh
//   gmsh -3 bp5_large.geo -setnumber res_f 5 -o bp5_large_med.msh

// --- Resolution parameters ---
DefineConstant[ res   = {80, Min 0, Max 2000, Name "Far-field resolution (km)"} ];
DefineConstant[ res_f = {10, Min 0, Max 1000, Name "Fault resolution (km)"} ];

// --- Geometric parameters (km) ---
// Domain half-sizes — 4x larger than standard BP5
Lx = 800;    // fault-normal half-width  (was 100, now matches Ly)
Ly = 800;    // along-strike half-width  (was 200)
Lz = 800;    // depth                    (was 100, now matches Lx and Ly)

// Fault dimensions (unchanged from SCEC BP5)
...
Field[2] = Threshold;
Field[2].InField = 1;
Field[2].SizeMin = res_f;      // At fault: res_f
Field[2].SizeMax = res;         // Far from fault: res (80 km)
Field[2].DistMin = 0;           // Start grading at fault
Field[2].DistMax = 200;         // Reach far-field size at 200 km distance (was 80)
