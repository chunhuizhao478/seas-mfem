# Implementation Plan: GPU Support for SEAS-MFEM (Dynamic + Quasi-Dynamic)

**Author:** code-plan agent
**Date:** 2026-05-16 (v1), revised 2026-05-16 (v1.1 with R-GPU-NNN fixes)
...
**Review status:** v1.1 incorporates 18 review findings from
[`gpu_implementation_plan_v1_check.md`](gpu_implementation_plan_v1_check.md);
each affected section is tagged with the corresponding `[R-GPU-NNN]`
identifier. Fix report: [`gpu_implementation_plan_v1_fix.md`](gpu_implementation_plan_v1_fix.md).
...
- HYPRE PCG+AMG GPU vs CPU on identical matrix: iteration counts may
  ...
  [R-GPU-007]
- Target speedup (NVIDIA A100, single GPU vs single CPU socket, BP5 500m):
  - End-to-end run: ≥ 3× wall-clock reduction.
  - Linear solve fraction: ≥ 5× reduction.
  - Friction loop fraction: ≥ 8× reduction.
- Memory: device buffers must not exceed 32 GiB (A100 80GB / MI250X 64GB
  per GCD give headroom). State vector + 8 ADER scratch buffers at TPV102
  default mesh = ~12 GiB at order 4.
...
### Goal
After Phase 0: every existing benchmark binary builds with `MFEM_USE_CUDA=YES`
(or `MFEM_USE_HIP=YES`), accepts a `--device <cuda|hip|cpu>` CLI flag, and
produces byte-identical results to the pre-change main when `--device cpu`
is selected. No physics changes yet.

### Files to Create
- `miniapps/seas/build_gpu_cuda.sh` — mirror of existing `build_frontera.sh`
  with CUDA flags; resolves `HYPRE_DIR`, `CUDA_ARCH`, `MFEM_USE_CUDA`,
  `MFEM_USE_MPI`, `MFEM_USE_PETSC` (if PETSc GPU available).
- `miniapps/seas/build_gpu_hip.sh` — HIP variant.
...
### Acceptance Criteria
- [ ] `make -C miniapps/seas` succeeds with `MFEM_USE_CUDA=YES`.
- [ ] `make -C miniapps/seas` succeeds with `MFEM_USE_HIP=YES`.
- [ ] All four drivers accept `--device cpu|cuda|hip|debug` and abort
      cleanly on unknown values.
- [ ] `test_device_init` passes on CPU build and CUDA build.
- [ ] TPV102 1-s smoke run with `--device cpu` is byte-identical to
      pre-change reference output.
- [ ] BP5 500m 10-step smoke run with `--device cpu` is byte-identical
      to pre-change reference output.
...
## Phase 7: Profiling, validation, and performance tuning

### Goal
After Phase 7: documented end-to-end speedups, baseline profile,
remaining hot spots identified for future work, and a regression
test suite that auto-runs on every CI commit.
...
- `miniapps/seas/document/gpu_dev/PERFORMANCE_BASELINE.md` — table of
  per-kernel speedups, occupancy, register pressure, memory bandwidth
  on A100, MI250X, V100.
- `miniapps/seas/document/gpu_dev/REGRESSION_CHECKLIST.md` — list of
  tests that must pass for any GPU-touching PR.

### Files to Modify
- `miniapps/seas/Makefile` — add `make perf` target that runs both
...
1. **Profiling markers.** Add `Nsight` ranges around each device
   kernel call using `nvtxRangePushA` / `nvtxRangePop`, wrapped in a
   `MFEM_USE_CUDA` `#ifdef`. AMD path: `roctxRangePush` / `roctxRangePop`.
...
3. **CI matrix.**
   ...
   - CUDA build: `--device debug` (host emulation) for correctness +
     `--device cuda` if a GPU runner is available.
4. **Baseline targets** (NVIDIA A100, single GPU, single MPI rank):
   - TPV102 200m 1-second: ≥ 5× wall-clock reduction vs single Xeon Gold core.
   - BP5 500m 100-step: ≥ 3× wall-clock reduction.
   - Friction kernel: ≥ 8× throughput.
   - Volume kernel: ≥ 6×.
   - Face flux kernel: ≥ 4× (penalty for atomics in Phase 4).
...
### Interfaces
- None new at API level. Profiling is internal.
...
- **Multi-GPU MPI** — `make perf NP=2,4,8` runs scaling study;
  document strong + weak scaling on TPV102 / BP5.

### Acceptance Criteria
- [ ] `make perf` produces a JSON timing table.
- [ ] `PERFORMANCE_BASELINE.md` is populated with measurements.
