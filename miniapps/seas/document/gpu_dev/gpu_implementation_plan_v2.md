# Implementation Plan: GPU Support for SEAS-MFEM (Dynamic + Quasi-Dynamic) — INCITE Edition

**Author:** code-plan agent
**Date:** 2026-05-16 (v1 + v1.1 + v2)
**Target branch:** `feature/gpu-port` (off `claude/priceless-carson-f1ea1b`)
**Predecessor (kept on disk):** [`gpu_implementation_plan_v1.md`](gpu_implementation_plan_v1.md) (v1.1, post-fix)
**Analysis seed:** [`gpu_porting_analysis_2026-05-16.md`](../system_dev/gpu_porting_analysis_2026-05-16.md)
**Supersedes:** `dynamic_rupture_plan_v4.md` §Phase 6 (was sketchy / never executed)
...
v2 deltas (this document) are tagged inline `[V2]` and summarized below.

## V1.1 → V2 changes summary

v1.1 assumed a single development GPU (NVIDIA A100). v2 retargets the
plan for an **INCITE allocation proposal** spanning DOE leadership
systems:

| Site | System | GPU | Programming model | INCITE? |
|------|--------|-----|-------------------|---------|
| OLCF | Frontier | AMD MI250X (2 GCDs/GPU, 4 GPUs/node = 8 GCDs/node) | HIP / ROCm | yes |
| ALCF | Aurora | Intel Data Center GPU Max (Ponte Vecchio, 2 stacks/GPU, 6 GPUs/node = 12 stacks/node) | SYCL / oneAPI | yes — gated, see §"Aurora gap" |
| ALCF | Polaris | NVIDIA A100 (4 GPUs/node) | CUDA | yes |
| OLCF | Summit | NVIDIA V100 (6 GPUs/node, CC 7.0) | CUDA | yes |

**Concrete v2 deltas:**

1. **New §"Target hardware matrix"** (after Constraints) — pins
   per-platform compiler / MPI / HYPRE / CUDA-arch / module tuples.
2. **Numerical / performance constraints** rewritten as a per-platform
   table; targets per device, not just A100.
3. **Phase 0 build infrastructure** now produces four platform build
   scripts (`build_polaris.sh`, `build_frontier_olcf.sh`,
   `build_summit.sh`, `build_aurora.sh` — last one gated on an
   MFEM CEED-SYCL bridge that does not currently exist; see §"Aurora
   gap"). The existing repo script is `build_frontera.sh` (TACC
   Frontera, a CPU system) — do not conflate with OLCF Frontier.
4. **Phase 7 baselines** expanded to four-column per-platform table;
   adds an **INCITE Readiness** deliverable (weak + strong scaling
   demo on Polaris + Frontier minimum, Aurora if gap closes,
   Summit for V100-class validation).
5. **Risk assessment** adds four new rows: Aurora SYCL gap,
   Summit V100 CC-7.0 atomic-CAS cost, MI250X dual-GCD partitioning,
   per-site HYPRE-GPU version drift.
6. **New Appendix D: INCITE node-hour budget** — ballpark estimate
   of development node-hours per phase and production node-hours for
   the proposal.
7. **§"Aurora gap" callout** (immediately after the hardware matrix)
   spells out the blocker and the decision tree: CEED-SYCL bridge,
   upstream MFEM SYCL, OpenMP fallback on Aurora Xeon Max CPU cores,
   or drop Aurora from the proposal.

v2 changes do NOT alter any Phase 1–6 kernel algorithms or interfaces.
All `[R-GPU-NNN]` review fixes remain in force.
...
- HYPRE PCG+AMG GPU vs CPU on identical matrix: iteration counts may
  ...
  [R-GPU-007]
- Target speedup, **per platform** (single device vs single CPU socket
  on the same node, BP5 500m for the linear-solve / end-to-end rows,
  TPV102 200m for the dynamic rows). Targets are conservative; Phase 7
  is allowed to revise them based on profile data. [V2]

  | Metric | Polaris A100 | Frontier MI250X (per GCD) | Summit V100 | Aurora PVC (per stack) |
  |---|---:|---:|---:|---:|
  | End-to-end BP5 500m | ≥ 3× | ≥ 2.5× | ≥ 1.5× | TBD (Aurora gap) |
  | HYPRE PCG+AMG solve | ≥ 5× | ≥ 4× | ≥ 2× | TBD |
  | Friction NR loop | ≥ 8× | ≥ 6× | ≥ 4× | TBD |
  | Volume kernel (TPV102) | ≥ 6× | ≥ 5× | ≥ 3× | TBD |
  | Face flux (atomic-add) | ≥ 4× | ≥ 3× | ≥ 1.2× (atomic-CAS) | TBD |
  | ADER predictor (TPV104) | ≥ 5× | ≥ 4× | ≥ 2× | TBD |

  Rationale for V100 lower bars: Volta (CC 7.0) has no hardware
  double-precision `atomicAdd` — MFEM's
  [`general/backends.hpp:69-91`](../../../general/backends.hpp) CAS-loop
  fallback applies. The face-flux atomic path is the worst-case here
  (~5–10× CAS overhead under contention); other kernels do not atomic
  and scale closer to the FP64 peak ratio. [V2]

- Memory budget, **per device** (single rank). State vector + 8 ADER
  scratch buffers at TPV102 default mesh ≈ 12 GiB at order 4. The
  smallest target is V100 (16 GiB), which constrains the largest
  per-rank problem size — TPV102 production runs targeting Summit
  must use at least 4× domain decomposition vs Frontier/Polaris.

  | Device | HBM | Effective per-rank cap |
  |---|---:|---:|
  | Polaris A100 (80GB SKU) | 80 GiB | 64 GiB |
  | Frontier MI250X (per GCD) | 64 GiB | 50 GiB |
  | Aurora PVC (per stack) | 64 GiB | 50 GiB |
  | Summit V100 (16 GiB SKU) | 16 GiB | 12 GiB |

  Per-platform cap drives the minimum np for each system; see
  Appendix D budget. [V2]
...
## Target hardware matrix [V2]

Each row is a (site, system, device, programming model) tuple that the
plan must support in build-system flags, runtime backends, and Phase 7
performance baselines. The "module / env" column records the per-site
toolchain version that the corresponding Phase 0 build script must
load.

| Site | System | Compute node | GPU device + arch flag | Programming model + MFEM backend | HYPRE GPU? | MPI flavour | Module / env (representative) | Notes |
|------|--------|--------------|------------------------|----------------------------------|------------|-------------|-------------------------------|-------|
| ALCF | **Polaris** | 1× AMD Milan + 4× NVIDIA A100 (40 GB HBM2) | `sm_80` | CUDA + `MFEM_USE_CUDA`, `Backend::CUDA` | yes (≥ 2.31, `--with-cuda`) | Cray MPICH (GPU-aware via `MPICH_GPU_SUPPORT_ENABLED=1`) | `module load PrgEnv-nvhpc cudatoolkit-standalone/12.x cray-hdf5-parallel` | Reference platform; closest to development laptop expectations. |
| OLCF | **Frontier** | 1× AMD EPYC + 4× MI250X (8 GCDs @ 64 GB HBM2e each) | `gfx90a` | HIP + `MFEM_USE_HIP`, `Backend::HIP` | yes (≥ 2.31, `--with-hip`) | Cray MPICH (GPU-aware via `MPICH_GPU_SUPPORT_ENABLED=1`) | `module load PrgEnv-amd amd/<roc> craype-accel-amd-gfx90a cray-hdf5-parallel` | Treat each MI250X GCD as a separate "GPU" — 8 GCDs/node, 8 MPI ranks/node. |
| ALCF | **Aurora** | 2× Intel Xeon Max + 6× PVC (12 stacks @ 64 GB HBM2e each) | `pvc` (Level Zero `bdf:0`/`bdf:1`) | SYCL / oneAPI + ***no MFEM backend yet*** — see §"Aurora gap" | experimental (HYPRE SYCL via Ginkgo / Kokkos, not via mainline) | Intel MPI / MPICH (GPU-aware via `ZE_FLAT_DEVICE_HIERARCHY=COMPOSITE`) | `module load oneapi/eng-compiler intel-pti hdf5/<v>` | Blocked on MFEM mainline; see Aurora gap. |
| OLCF | **Summit** | 2× IBM POWER9 + 6× V100 (16 GB HBM2) | `sm_70` | CUDA + `MFEM_USE_CUDA`, `Backend::CUDA` | yes (≥ 2.31, `--with-cuda`) | Spectrum MPI (GPU-aware via `--smpiargs="-gpu"` jsrun flag) | `module load cuda/12.x gcc/12 spectrum-mpi hdf5/<v>` | V100 lacks fast FP64 atomicAdd (CC 7.0) — see Risk Assessment. |

**Programming-model invariant.** Phase 1–6 kernels are written once
against MFEM's `mfem::forall` + `Vector::Read/Write` API. MFEM
dispatches the same kernel source through CUDA on Polaris/Summit,
through HIP on Frontier, and through (eventually) a SYCL bridge on
Aurora. The application code does NOT contain CUDA/HIP-specific
`#ifdef`s — those live inside MFEM. The build scripts (Phase 0)
select the backend at MFEM configure time.

## Aurora gap [V2]

**Status:** Aurora support is BLOCKED at the time of writing.

**The blocker:** [`general/device.hpp:33-79`](../../../general/device.hpp)
enumerates MFEM's backend list. It contains `CUDA`, `HIP`,
`RAJA_CUDA`, `RAJA_HIP`, `OCCA_CUDA`, `CEED_CUDA`, `CEED_HIP`, and
`DEBUG_DEVICE`. There is **no SYCL backend, no `CEED_SYCL` backend,
and no Level Zero backend**. A grep of MFEM's `general/`, `linalg/`,
and `config/` directories returns zero matches for `sycl` / `SYCL`.
HYPRE's SYCL execution policy (≥ 2.32, experimental) is reachable
through `Hypre::InitDevice` but only if the surrounding MFEM device
context understands a SYCL backend — which it does not.

**Aurora porting options, ranked by realism:**

1. **CEED-SYCL via a new MFEM `Backend::CEED_SYCL`** (most realistic).
   libCEED's SYCL backend exists and runs on Aurora PVC
   ([oneAPI DevSummit 2023 report](https://simplecore.intel.com/oneapi-io/wp-content/uploads/sites/98/Umesh-Kris-Varsha-Development-and-Optimization-of-a-SYCL-Backend-for-libCEED.pdf)).
   Adding `CEED_SYCL` to MFEM mirrors the existing `CEED_CUDA` /
   `CEED_HIP` pattern. **However, our Phase 3–5 kernels are not
   written through CEED's QFunction API** — they are hand-rolled
   `mfem::forall` kernels. To dispatch them on Aurora through CEED,
   we'd have to re-author the kernels as CEED QFunctions. This is a
   ~Phase 4–6 worth of rework (~2–3 months on top of v1.1's existing
   estimate).
2. **Upstream MFEM SYCL backend.** Out of our hands; MFEM project has
   not announced a SYCL backend roadmap as of 2026-05. Hope, but do
   not plan on, this happening.
3. **OpenMP-on-Xeon-Max** fallback on Aurora. Aurora compute nodes
   carry 2× Xeon Max CPUs with HBM. Running our `--device cpu` path
   on Aurora's CPU cores uses the node's HBM but ignores the 6 PVC
   GPUs. This is a Plan-B fallback for the INCITE proposal: we can
   still claim "we run on Aurora" using CPU-only.
4. **Drop Aurora.** Submit INCITE for Polaris + Frontier (+ Summit)
   only. Acceptable if reviewers do not penalize platform breadth.

**Decision (this plan):** Phase 0 produces a stub `build_aurora.sh`
script that configures MFEM in CPU-only mode (`MFEM_USE_CUDA=NO,
MFEM_USE_HIP=NO`) so we can at least demonstrate the BP5 / TPV
benchmarks on Aurora's Xeon Max CPUs. Per-platform performance table
in §"Numerical / performance constraints" leaves Aurora PVC entries
as `TBD (Aurora gap)`. Phase 7 §"INCITE Readiness deliverable"
includes an Aurora-CPU scaling demo separately from the GPU demos.

**Re-evaluation trigger:** if the INCITE program officer rejects the
proposal for "insufficient Aurora support" or MFEM upstream lands a
SYCL backend before the resubmission window, escalate options 1 or 2
into a separate Phase 8 plan.
...
### Files to Create
Four platform build scripts, mirroring the pattern of the existing
[`build_frontera.sh`](../../../build_frontera.sh) (which targets TACC
Frontera, a Skylake CPU system — do not confuse with OLCF Frontier).
Each script: (a) loads the site modules, (b) verifies required
environment variables, (c) calls `make config` with site-specific
flags, (d) builds MFEM + the four drivers. **All four scripts live
at the repo root** (sibling to `build_frontera.sh`), not in
`miniapps/seas/`, because they configure the MFEM library itself.
[V2]

- **`build_polaris.sh`** — Polaris (ALCF, NVIDIA A100, CUDA):
  ```bash
  ...
  ```
- **`build_frontier_olcf.sh`** — Frontier (OLCF, AMD MI250X, HIP):
  ```bash
  ...
  make -j16 && cd miniapps/seas && make -j16
  ```
- **`build_summit.sh`** — Summit (OLCF, NVIDIA V100, CUDA):
  ```bash
  module load cuda/12.x gcc/12 spectrum-mpi hdf5/<v>
  export CUDA_ARCH=sm_70   # V100; lacks fast FP64 atomicAdd
  make config \
    MFEM_USE_MPI=YES MFEM_USE_CUDA=YES MFEM_USE_METIS=YES \
    MFEM_USE_LAPACK=YES \
    CUDA_ARCH=$CUDA_ARCH \
    HYPRE_OPT="-I${OLCF_HYPRE_ROOT}/include" \
    HYPRE_LIB="-L${OLCF_HYPRE_ROOT}/lib -lHYPRE"
  make -j16 && cd miniapps/seas && make -j16
  ```
  Build script must include a `WARNING:` echo at the top noting that
  Summit V100 (CC 7.0) uses MFEM's CAS-loop FP64 atomicAdd fallback —
  face-flux kernel performance will be lower than CC 6.0+ targets.
- **`build_aurora.sh`** — Aurora (ALCF, Intel PVC, CPU-only fallback
  pending §"Aurora gap" resolution):
  ```bash
  module load oneapi/eng-compiler intel-pti hdf5/<v>
  # NO MFEM_USE_CUDA / MFEM_USE_HIP — CPU-only on Xeon Max
  # MFEM_USE_SYCL does not exist in mainline MFEM as of 2026-05.
  make config \
    MFEM_USE_MPI=YES MFEM_USE_OPENMP=YES MFEM_USE_METIS=YES \
    MFEM_USE_LAPACK=YES \
    CXX=mpiicpx CC=mpiicx \
    HYPRE_OPT="-I${HYPRE_DIR}/include" \
    HYPRE_LIB="-L${HYPRE_DIR}/lib -lHYPRE"
  make -j16 && cd miniapps/seas && make -j16
  ```
  Header comment in this script must explicitly state Aurora GPU
  support is BLOCKED on the §"Aurora gap" resolution; the script
  produces a CPU-only binary that runs on Xeon Max but ignores the
  PVC devices. Update this script when CEED_SYCL or MFEM SYCL lands.
...
### Acceptance Criteria
- [ ] `build_polaris.sh` builds MFEM + SEAS on a Polaris compute node
      (or login node with `gpu` queue access); produces a binary that
      runs `tpv102_driver --device cuda --tfinal 0.01` to completion
      on 1 A100. [V2]
- [ ] `build_frontier_olcf.sh` builds MFEM + SEAS on Frontier;
      produces a binary that runs `tpv102_driver --device hip
      --tfinal 0.01` to completion on 1 MI250X GCD. [V2]
- [ ] `build_summit.sh` builds MFEM + SEAS on Summit; produces a
      binary that runs `tpv102_driver --device cuda --tfinal 0.01`
      to completion on 1 V100 (NOTE the CC-7.0 warning emitted by
      the script). [V2]
- [ ] `build_aurora.sh` builds MFEM + SEAS on Aurora in CPU-only
      mode; produces a binary that runs `tpv102_driver --device cpu
      --tfinal 0.01` to completion on 1 Xeon Max socket (PVC GPUs
      idle pending Aurora gap resolution). [V2]
- [ ] All four drivers accept `--device cpu|cuda|hip|debug` and abort
      cleanly on unknown values.
- [ ] `test_device_init` passes on CPU build and CUDA build (and
      HIP build, on a Frontier login node or local AMD test box).
- [ ] TPV102 1-s smoke run with `--device cpu` is byte-identical to
      pre-change reference output on EACH platform (Polaris, Frontier,
      Summit, Aurora) — uses the same CPU code path on all. [V2]
- [ ] BP5 500m 10-step smoke run with `--device cpu` is byte-identical
      to pre-change reference output on each platform.
...
## Phase 7: Profiling, validation, performance tuning, and INCITE readiness [V2]

### Goal
After Phase 7: documented end-to-end speedups on Polaris (CUDA),
Frontier (HIP), Summit (CUDA), and Aurora (CPU fallback);
INCITE-grade scaling demonstration (weak + strong) on at least
Polaris + Frontier; per-kernel performance baseline; regression
test suite auto-running in CI; and a populated `INCITE_READINESS.md`
deliverable suitable for inclusion in an INCITE proposal narrative.
[V2]

### Files to Create
- `miniapps/seas/tests/perf/perf_tpv102_device.cpp` — micro-benchmark:
  - 10 RK4 stages, no output.
  - Reports per-kernel time (Nsight / rocprof markers).
...
- `miniapps/seas/tests/perf/scaling_tpv102.cpp` — weak + strong
  ...
  fraction. Used by the INCITE readiness deliverable. [V2]
- `miniapps/seas/tests/perf/scaling_bp5.cpp` — same for BP5
  quasi-dynamic. [V2]
- `miniapps/seas/document/gpu_dev/PERFORMANCE_BASELINE.md` — **per
  platform** table of per-kernel speedups, occupancy, register
  pressure, memory bandwidth on Polaris (A100), Frontier (MI250X
  per GCD), Summit (V100), Aurora (Xeon Max CPU). [V2]
...
- `miniapps/seas/document/gpu_dev/INCITE_READINESS.md` — proposal-ready
  document containing: (a) single-node achievable performance per
  platform, (b) weak-scaling plots up to 1024 nodes per platform,
  (c) strong-scaling plots at fixed problem size, (d) node-hour
  estimate justifying the requested allocation. Sourced from
  Appendix D of this plan. [V2]
- `miniapps/seas/document/gpu_dev/CI_MATRIX.md` — documents the
  ...
  Implementer must first determine which CI system this repo uses
...
2. **Per-kernel timing collection.** Use `mfem::tic()` / `mfem::toc()`
   ...
   reduce noise.
3. **CI matrix (separate from on-site INCITE runs):**
   - CPU build: existing tests + `--device cpu` smoke.
   - CUDA build (on a local NVIDIA dev box): `--device debug` (host
     emulation) for correctness + `--device cuda` if a GPU runner is
     available.
   - HIP build (if AMD dev box is available): same, with `--device hip`.
4. **Per-platform baseline targets.** Targets come from the
   per-platform table in §"Numerical / performance constraints". The
   Phase 7 deliverable confirms each row, OR adjusts the targets in
   the plan and re-runs (a target miss is data, not a failure). [V2]

   | Kernel | Polaris A100 | Frontier MI250X (per GCD) | Summit V100 | Aurora PVC |
   |---|---:|---:|---:|---:|
   | Friction NR loop | ≥ 8× | ≥ 6× | ≥ 4× | Aurora gap |
   | Volume RHS | ≥ 6× | ≥ 5× | ≥ 3× | Aurora gap |
   | Mass inverse | ≥ 6× | ≥ 5× | ≥ 3× | Aurora gap |
   | ADER predictor | ≥ 5× | ≥ 4× | ≥ 2× | Aurora gap |
   | Face flux (atomic) | ≥ 4× | ≥ 3× | ≥ 1.2× (CAS) | Aurora gap |
   | HYPRE PCG+AMG | ≥ 5× | ≥ 4× | ≥ 2× | Aurora gap |
   | End-to-end BP5 500m | ≥ 3× | ≥ 2.5× | ≥ 1.5× | Aurora gap |
   | End-to-end TPV102 1s | ≥ 5× | ≥ 4× | ≥ 2× | Aurora gap |

5. **INCITE Readiness deliverable** (populates `INCITE_READINESS.md`):
   - Single-node achievable: TPV102 200m, BP5 500m on each platform,
     1 device (1 A100, 1 MI250X GCD, 1 V100, 1 Xeon Max socket).
     Report wallclock, memory peak, GPU utilisation %.
   - Strong scaling: TPV102 200m at fixed problem size, np ∈
     {1, 2, 4, 8, 16, 32, 64, 128} on Polaris and Frontier minimum;
     plot speedup vs np and parallel efficiency.
   - Weak scaling: TPV102 with mesh refinement keeping DOFs/rank
     constant, np ∈ {1, 4, 16, 64, 256, 1024} on Polaris and
     Frontier minimum; plot time/step vs np.
   - For Summit: same demos at smaller np (≤ 32) because Summit
     allocations are typically smaller.
   - For Aurora (CPU fallback): same demos but `--device cpu` on
     Xeon Max sockets, np up to 16 nodes minimum. Flag the
     comparison-with-PVC gap explicitly in the readiness doc.
6. **Document remaining gaps:**
   ...
   - Custom DG integrator PA path for BP5 stiffness assembly (still
     host).
   - **Aurora SYCL backend** (mainline MFEM does not have one — see
     §"Aurora gap"). [V2]
...
- **Multi-GPU MPI** — `make scaling NP=2,4,8,16,32,64,128` runs
  strong-scaling sweep; documented in `INCITE_READINESS.md`. [V2]
- **MI250X dual-GCD per device** — Frontier scaling tables MUST
  report np in GCDs, not GPUs. 1 MI250X = 2 GCDs; an 8-GCD node
  has 4 physical GPUs. Mismatching units is a common reporting
  bug in INCITE proposals. [V2]
- **Summit V100 atomic-CAS regression vs A100** — face-flux kernel
  on V100 may show ~5–10× slowdown vs A100 even though the rest of
  the operator is closer. Document this as a known cost in the
  baseline doc; do not "fix" by abandoning V100 — the demo still
  shows portability. [V2]
- **Aurora PVC gap** — Aurora rows in scaling tables are "CPU only"
  pending §"Aurora gap" resolution. Explicit caveat in the readiness
  doc text. [V2]

### Acceptance Criteria
- [ ] `make perf` produces a JSON timing table on each available
      platform.
- [ ] `make scaling NP=...` produces strong + weak scaling JSON on
      Polaris + Frontier (minimum). [V2]
- [ ] `PERFORMANCE_BASELINE.md` is populated with measurements for
      Polaris, Frontier, Summit, Aurora (CPU). [V2]
- [ ] `INCITE_READINESS.md` is populated with single-node, strong,
      and weak scaling figures + a node-hour estimate matching
      Appendix D. [V2]
- [ ] All Phase 0–6 acceptance tests in the regression checklist pass
      on all four platforms (Aurora limited to CPU path). [V2]
...
| HYPRE built without GPU but `--device cuda` requested | High | Med | Phase 1 warning + silent CPU solve; explicit detection of `GetHypreMemoryClass() == HOST`. |
| Atomics on `real_t` slow on A100 / MI250X | Med | Med | Phase 4 uses atomics; Phase 7 switches to colour-pass / element-gather if profile shows > 30% time in atomics. |
| CUDA-aware MPI not available on target cluster | Med | High | Phase 4 provides device↔host bridge with a clear `PERF[G-401]` marker. |
...
| Per-element register pressure overflow at p > 4 | Low | Med | Phase 3 aborts at `ndof > MAX_DOF`; Phase 7 shared-memory tiling path planned. |
...
| **[V2] Aurora SYCL gap — no MFEM SYCL backend exists** | High | High | Phase 0 ships a CPU-only `build_aurora.sh`; INCITE proposal carries Aurora as "CPU readiness only" with a Plan-B note. Re-evaluate if MFEM upstream adds SYCL or if we author a `Backend::CEED_SYCL` bridge in Phase 8. |
| **[V2] Summit V100 (CC 7.0) FP64 atomicAdd uses MFEM CAS-loop fallback** | High | Med | Face-flux speedup target reduced to ≥ 1.2× on V100 (vs ≥ 4× on A100). Documented in §"Numerical / performance constraints" and Phase 7 baseline. Switch to colour-pass scatter (Phase 4 option B) is the principal mitigation if V100 face flux dominates wall time. |
| **[V2] MI250X dual-GCD partitioning — reporting np in "GPUs" vs "GCDs" inconsistency** | Med | Med | Phase 7 scaling acceptance criterion explicitly requires np reported in GCDs (1 MI250X = 2 GCDs; 8 GCDs/node = 4 physical GPUs). All Frontier plots in `INCITE_READINESS.md` use GCDs. |
| **[V2] Per-site HYPRE-GPU version drift** (Polaris vs Frontier vs Summit may carry different HYPRE micro-versions; AMG smoother / coarsening defaults differ across HYPRE 2.31, 2.32, 2.33) | Med | Med | Phase 1 acceptance loosened to "GPU and CPU AMG within 2× iteration count, `< 1e-6` L2 diff" already covers this; per-platform `PERFORMANCE_BASELINE.md` records the HYPRE version per site. |
...
## Appendix D: INCITE node-hour budget [V2]
...
the INCITE proposal narrative. All numbers are **planning ballparks**,
to be refined once Phase 7 produces single-node measurements.

### Per-platform node-hour conversion

INCITE awards node-hours, not GPU-hours. One node-hour =
all-devices-on-one-node-busy-for-one-wallclock-hour.

| Platform | GPUs/node (effective compute units) | "1 node-hour" ≈ |
|----------|-------------------------------------|-----------------|
| Polaris (A100) | 4 GPUs | 4 GPU-hours |
| Frontier (MI250X) | 8 GCDs (= 4 physical MI250X) | 8 GCD-hours |
| Summit (V100) | 6 GPUs | 6 GPU-hours |
| Aurora (PVC) | 12 stacks (= 6 physical PVC) | 12 stack-hours (CPU fallback uses 2 Xeon Max sockets ≈ 104 cores) |

### Development node-hours per phase (estimate)

These are **dev-time** runs needed to land each phase: short
benchmarks, regression sweeps, parameter studies. They include
re-runs after debugging.

| Phase | Activity | Polaris | Frontier | Summit | Aurora (CPU) | Total dev |
|-------|---------|--------:|---------:|-------:|-------------:|----------:|
| 0 | Build & smoke (1-s TPV102 + 10-step BP5 on each platform, ~50 attempts including build retries) | 50 nh | 50 nh | 30 nh | 30 nh | 160 nh |
| 1 | BP5 HYPRE GPU solve (~200 cg+amg short solves at 500m) | 200 nh | 200 nh | 100 nh | n/a | 500 nh |
| 2 | Friction NR kernel (~500 micro-tests + 50 BP5 100-step runs) | 100 nh | 100 nh | 50 nh | n/a | 250 nh |
| 3 | Volume + mass-inverse + ApplySpatialDerivative (~100 TPV102 200m short runs) | 200 nh | 200 nh | 100 nh | n/a | 500 nh |
| 4 | Face flux on GPU — hardest phase, expect ~500 short runs to debug bucket dispatch + atomics + shared seam | 1000 nh | 1000 nh | 300 nh | n/a | 2300 nh |
| 5 | ADER on GPU (~200 TPV104 substep runs) | 500 nh | 500 nh | 200 nh | n/a | 1200 nh |
| 6 | BP5 RHS + traction recovery (~100 BP5 1000-step runs) | 300 nh | 300 nh | 100 nh | n/a | 700 nh |
| 7 | Profiling + scaling (np ∈ {1,2,4,8,16,32,64,128,256} weak + strong, several reps) | 1500 nh | 1500 nh | 500 nh | 300 nh | 3800 nh |
| **Dev subtotal** | | **3850 nh** | **3850 nh** | **1380 nh** | **330 nh** | **9410 nh** |

Total development budget: **≈ 10k node-hours**, dominated by Phase 4
(face flux) and Phase 7 (scaling demo). A reasonable ASK for the
*development* portion of an INCITE proposal would be **15k–20k
node-hours total** across all sites (1.5× safety factor + buffer for
re-running after review fixes).

### Production node-hours for the INCITE proposal

The INCITE proposal narrative needs to justify a production-run
budget for the science the GPU port enables. Two example
science campaigns:

**Campaign 1 — BP5 parameter study (quasi-dynamic):**
- 12 parameter combinations (Vp ∈ {1e-12, 1e-11, 1e-10 m/s} × Wf ∈
  {30, 40, 50, 60 km}).
- Each run: 100 simulated years, ~10⁷ time steps at the current BP5
  500m resolution. Expected wallclock at Polaris ~3× speedup vs CPU:
  ~24 hours on 32 nodes = 768 node-hours per run.
- Total: 12 runs × 768 nh = **~9.2k node-hours**. Multiply by 2 for
  re-run / mesh-refinement budget → **~20k node-hours**.

**Campaign 2 — TPV104 high-resolution rupture sweep (dynamic):**
- 6 nucleation configurations × 3 mesh resolutions (200m, 100m, 50m).
- Each TPV104 60-s run at 100m on 128 GCDs of Frontier with ~4× GPU
  speedup: ~6 hours wallclock = 96 node-hours.
- The 50m resolution doubles per dimension → 8× more DOFs, 16× more
  wallclock per step (with smaller dt for CFL). Estimate ~1500 nh
  per 50m run.
- Total: 18 runs at mixed res ≈ **~12k node-hours**. With safety
  factor → **~25k node-hours**.

**Campaign 3 — TPV102 long-time validation (dynamic):**
- 4 ADER orders × 3 frictions (RS / FVW / LSW from TPV102 / 104 / 205
  setups) × 2 PML widths.
- 24 runs × ~200 nh each at production resolution → **~5k node-hours**.

**Production subtotal:** ~50k node-hours across the three campaigns.

### Suggested INCITE ask

Combining dev + production, plus a 25% margin for unanticipated
runs and Phase 7 baseline re-measurement:

| Site | Suggested ask | Why this site |
|------|--------------:|--------------|
| Polaris (ALCF) | 25,000 node-hours | Primary CUDA dev + Campaign 1 |
| Frontier (OLCF) | 35,000 node-hours | Primary HIP dev + Campaign 2 (largest meshes, dual-GCD) |
| Summit (OLCF) | 5,000 node-hours | Portability validation + Campaign 3 partial |
| Aurora (ALCF) | 3,000 node-hours (CPU only initially; grow if SYCL gap closes) | CPU-fallback demonstration |
| **Total** | **68,000 node-hours** | |

This is well within typical INCITE allocation sizes (which range from
tens of thousands to ~10 million node-hours). The proposal narrative
should:
- Cite the dev budget separately from the production budget.
- Acknowledge the Aurora gap explicitly and propose a separate
  Director's Discretionary or ASCR Leadership Computing Challenge
  (ALCC) ask if the SYCL gap closes mid-allocation.
- Reference the §"Per-platform baseline targets" table in this plan
  as the readiness evidence.

### Re-cost trigger

If Phase 7 single-node measurements show ≥ 2× discrepancy from the
speedup targets in §"Numerical / performance constraints", revisit
this appendix and reset the production budget proportionally BEFORE
submitting the INCITE proposal.
