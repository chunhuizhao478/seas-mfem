...
### Phase H Stage 2 — per-element volume / BC / fault dispatch

Per `debug_document/general_driver_debug_document/PLAN_heterogeneous_volume_bc_fault_dispatch_2026-05-20.md`, the heterogeneous `(MaterialField, BoundaryConfig)` ctor now drives per-element material through THREE further code paths beyond the Phase-R interior faces, via the single dispatch helper `FluxForElem_(e)` (returns `owned_flux_pool_->At(e)` when the pool is built, else the scalar `flux_`):

- **Group A — volume + ADER CK Jacobians**: `ComputeVolumeRHS` fetches `FluxForElem_(e).GetReferenceStarMatrix(0/1/2)` per element; the ADER Cauchy-Kovalevskaya recursion (`ComputeADERTimeIntegrated`, `ComputeADERSubStepStates`) routes through the per-element `ApplyJacobianPerElementDOF_` instead of the single-matrix `ApplyJacobianPerDOF`.
- **Group B — boundary-face flux**: `Absorbing/FreeSurface/FreeSurfaceGodunov` (RK4 + ADER) use `FluxForElem_(e1)` (BC faces are one-sided; `e1` is the owning element).
- **Group C — fault bulk-side flux**: the four `A_n·Q_imp` sites use `FluxForElem_(elem_plus/elem_minus)` for interior faults (`e1`/`e2` per `interior_fault_elem1_on_plus_`) and `FluxForElem_(e1)` for shared faults (only the local element lives on-rank).  The friction solve and imposed-state construction stay VERBATIM — they were already per-DOF heterogeneous-ready (`FaultFaceFlux` reads impedances from `DOFData`).

Prerequisite: `GodunovFluxPool::Build` now builds each cached `GodunovFlux` from the EXACT `(λ,μ,ρ)` (rounding only the dedup key), so `At(e)` is bit-identical to the scalar `flux_` on `Mode::Constant` — the byte-parity gate holds even for >6-sig-fig constants.

Consequently the old R-002 guard (`Mode::Coefficient` + real BC → abort) is RELAXED: `depth_profile_1d` materials (e.g. TPV31) run end-to-end through `seas_spatial_dyn_driver`.  STILL rejected: `Mode::GridFunction` (no centroid eval) and `MixedFluxMode != None`.  True bi-material ACROSS the fault stays out of scope, guarded at run time by the `Zp_plus == Zp_minus` assertions in `FaultFaceFlux`.  Every scalar-ctor driver (TPV205/102/104, BP5) is byte-untouched (`owned_flux_pool_` is null → `FluxForElem_` returns `flux_`).  Gates: `test_phaseh_wave_operator_constant_parity` (extended: byte-exact volume + ADER-CK predictor, FP-equivalent Mult + AdvanceADER under mixed BC + fault, clean and >6-sig constants) and `test_phaseh_heterogeneous_correctness` (H-1/H-2/H-3).
