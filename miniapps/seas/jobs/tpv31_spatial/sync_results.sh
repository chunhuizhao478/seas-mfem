#!/bin/bash
# =============================================================================
# Pull TPV31 spatial-dyn STATION traces + the fault ParaView file from
# Frontera into a local archive tree.
#
# Run this on your LOCAL machine (Mac), not on Frontera.
#
# Auto-detects every `results_spatial_dyn_*` directory under
#   <REMOTE_ROOT>/miniapps/seas/tpv31/
# and rsync's each one to a same-named local subdir.  Reproduces the cluster's
# directory layout but transfers ONLY the station *.dat traces and the single
# `fault.vtkhdf` ParaView file (the large volume .vtkhdf, SLURM logs, meshes,
# and source are intentionally skipped).
#
# Default destination:
...
#   ./sync_results.sh 7738863                        # only matching jobs
#   ./sync_results.sh 7738863 7740000                # multiple
...
#   *.dat          (SCEC station traces — e.g. tpv31_station_faultst000dp075.dat)
#   fault.vtkhdf   (fault-plane ParaView output, single VTKHDF file per run)
#
# Not transferred:
#   volume .vtkhdf (large), SLURM stdout/stderr, .msh meshes, RESULT.txt,
#   source code, anything else.
# =============================================================================

set -u

REMOTE_ROOT="/scratch2/10024/zhaochun/seas-project/seas-mfem-heterogeneous-riemann-solver/miniapps/seas/tpv31"
DEFAULT_DEST="$HOME/Downloads/seas-mfem/tpv31_spatial"
...
# directory name (no path) like "results_spatial_dyn_50m_p1_O2_dev_job7738863".
