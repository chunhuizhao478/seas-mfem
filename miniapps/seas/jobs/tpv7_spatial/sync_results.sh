#!/bin/bash
# =============================================================================
# Pull TPV7 SPATIAL dynamic-rupture result files from Frontera into a local
# archive tree for ParaView / on-fault-station inspection.
#
# TPV7 is the LOW-CONTRAST bi-material variant of TPV6 (same geometry and
# per-side on-fault station layout, smaller material jump across the fault).
#
...
# For TPV7 the jobs name them e.g.  out_p1_rk45_mixedflux_200m_<jobid>  (mixed
...
#
# TPV7 emits PER-SIDE on-fault station traces (the shared TPV6/7 station writer,
...
#   ./sync_results.sh --dest /Volumes/SSD/seas/tpv7_spatial
#   ./sync_results.sh --host zhaochun@frontera.tacc.utexas.edu
#   ./sync_results.sh --remote-root /scratch2/.../miniapps/seas/tpv7    # non-default checkout
#
...
# fix-cross-rank-material-exchange branch.  TPV7 lives only on this branch, so
...
# Remote root = <checkout>/miniapps/seas/tpv7 (--remote-root overrides the whole path).
