#!/bin/bash
# =============================================================================
# Pull TPV102 SPATIAL dynamic-rupture result files from Frontera into a local
...
# For TPV102 the dev/prod jobs name it  out_rk45_mf_<jobid>  (and out_<jobid>
# for the bare dev smoke).  This script auto-detects every `out_*` dir and
# rsync's the THREE artefact families the on-fault analysis needs into a
# same-named local subdir:
#
...
#   (3) Station traces *.dat                                     (SCEC on-fault stations)
#
# The LARGE volume field (volume.vtkhdf / volume/*.vtu from --paraview) is NOT
# transferred by default — pass --volume to add it.
#
# Default destination:
#   $HOME/Downloads/seas-mfem/tpv102_spatial
#   ( `_spatial` suffix keeps these separate from the temporal-driver archive
#     at $HOME/Downloads/seas-mfem/tpv102 written by jobs/tpv102/sync_results.sh )
# Override with --dest <path> or the LOCAL_DEST environment variable.
#
# Usage:
#   ./sync_results.sh                                # all out_* dirs (fault vtu + pvd + .dat)
#   ./sync_results.sh 7760146                        # only matching job(s)
#   ./sync_results.sh 7760146 7760200                # multiple
#   ./sync_results.sh --volume                       # also pull the volume field (large)
#   ./sync_results.sh --dry-run                      # preview, transfer nothing
#   ./sync_results.sh --dest /Volumes/SSD/seas/tpv102_spatial
#   ./sync_results.sh --host zhaochun@frontera.tacc.utexas.edu
#   ./sync_results.sh --remote-root /scratch2/.../miniapps/seas/tpv102   # non-default checkout
#
...
FILE_INCLUDES=(
    ...
    --include='*.dat'                # SCEC on-fault station traces
)
