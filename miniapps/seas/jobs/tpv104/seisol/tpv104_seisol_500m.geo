// SCEC TPV104 SeisSol mesh, 500 m fault resolution — paired with MFEM
// `tpv104/mesh/tpv104_500m.geo` (byte-identical geometry; only Physical
// Surface tags differ to match SeisSol's PUMGen convention).  See
// `jobs/tpv104/seisol/README.md` for the build flow:
//   gmsh -3 -optimize tpv104_seisol_500m.geo -o tpv104_seisol_500m.msh
//   pumgen -s msh2 tpv104_seisol_500m.msh tpv104_seisol_500m
//
// SeisSol PhysicalSurface convention (per SeisSol docs/gmsh.rst):
//   101 = free surface
//   103 = dynamic-rupture (fault)
//   105 = absorbing
// MFEM's paired tpv104_500m.geo uses 1/3/5; otherwise byte-identical
// geometry.
//
// Geometry (SCEC TPV104, half-space):
...
//
// Physical groups (match current TPV104 driver defaults):
...
//
// Usage:
//   gmsh -3 tpv104_500m.geo -o tpv104_500m.msh
...
Physical Surface(101) = {1};                  // free surface (Z=0 top)
Physical Surface(103) = {120, 121, 122, 123, 124, 125, 200}; // fault (segmented VW/VS regions + nucleation patch)
