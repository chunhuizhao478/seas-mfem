...
// PAD_TOP defaults to 0 so the box top face sits at z = 0 exactly
// (the cleaned STL has fault_zmax = 0).  The fault top edge is then
// co-planar with the free surface and shares its boundary curve;
// gmsh's discrete `Surface{fault_tag} In Volume{1}` still meshes
// without ClassifySurfaces because the box top is a single planar
// face of the OCC volume.
//
...
// pad_top default = 0 m (free surface at z = 0 exactly);
// pad_bottom default = 25 km.
PAD_XY     = 50000.0;   // 50 km horizontal margin (legacy default)
PAD_TOP_DEFAULT    = 0.0;         // box top = fault_zmax (= 0 in cleaned STL)
...
If (USE_Z_GRADED == 1)
    ...
    bg_raw_field_id = 9;
Else
    bg_raw_field_id = bg_field_id;
EndIf

// -------------------------------------------------------------
// PLAN_mesh_quality.md Phase 2 -- hard size floor.
// Field[10] is a constant size = lc_min everywhere.  Field[11] is the
// per-point Max of (background_raw, lc_min) so the final background
// LC is bounded below by lc_min.  This is the only robust counter-
// measure against gmsh's embedded-discrete-surface code path, which
// overrides Mesh.CharacteristicLengthMin near the merged fault and
// can otherwise produce <100 m bulk edges at the fault top.
// -------------------------------------------------------------
If (!Exists(lc_min))  lc_min = 100.0;  EndIf
Field[10] = MathEval;
Field[10].F = Sprintf("%g", lc_min);
Field[11] = Max;
Field[11].FieldsList = {bg_raw_field_id, 10};
Background Field = 11;
...
// -------------------------------------------------------------
// PLAN_mesh_quality.md Phase 3 -- quality optimization.
// gmsh's Mesh.Optimize is a Laplacian-style edge-swap + node relocation
// pass; it respects the embedded fault topology automatically.
// Mesh.OptimizeNetgen is more aggressive but EMPIRICALLY CRASHES with
// SIGBUS during 'Remove Illegal Elements' on this geometry (embedded
// discrete fault + lc_min floor), so it is OFF by default per the plan's
// Risk Assessment fallback.  Re-enable explicitly via
// --do-optimize-netgen / -setnumber do_optimize_netgen=1 only on a
// gmsh build known to handle embedded surfaces robustly.
//
// OptimizeThreshold is gmsh's `gamma` metric (Inradius/Circumradius);
// 0.3 corresponds roughly to Joe-Liu eta ~ 0.5, well above our eta > 0.1
// acceptance bar.  Smoothing passes default to 10 (up from gmsh's
// default of 1) to compensate for Netgen being off.
// All four knobs are overridable via -setnumber.
// -------------------------------------------------------------
If (!Exists(do_optimize))         do_optimize         = 1;   EndIf
If (!Exists(do_optimize_netgen))  do_optimize_netgen  = 0;   EndIf
If (!Exists(optimize_threshold))  optimize_threshold  = 0.3; EndIf
If (!Exists(smoothing_passes))    smoothing_passes    = 10;  EndIf
