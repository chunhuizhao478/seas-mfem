"""build_velocity_cvmh.py — top-level driver: CVM-H ASCII -> sidecar.
...
schema-v1 sanity guards (G-1: no NaN; G-2: per-field min/max; G-3:
optional mesh-bbox containment), and writes the schema-conforming
HDF5 sidecar.
...
::
    ...
    python -m data_projection.build_velocity_cvmh \\
        ...
        --grid-dx       1500 \\
        --vp-min-mps    2000 --vp-max-mps  9000 \\
        --vs-min-mps    1500 --vs-max-mps  5000 \\
        --rho-min-kgm3  2000 --rho-max-kgm3 3500 \\
...
def _encode_b64_appended(arrays: list) -> tuple:
    """Pack a list of float64 numpy arrays into a single base64 string
    using VTK's "header (uint64 byte_count) + raw bytes" per array,
    concatenated.  Returns (b64_str, offsets) where offsets[i] is the
    byte offset of the i-th array's HEADER from the start of the
    concatenated payload (this is what VTK XML's `offset=N` requires).
    """
    ...
    for arr in arrays:
        ...
        payload.extend(raw)
    return base64.b64encode(bytes(payload)).decode("ascii"), offsets
...
def _write_vti_companion(path: Path,
    ...
    arrays = [arr.transpose(2, 1, 0).ravel(order="C")
              for arr in fields.values()]
    payload_b64, offsets = _encode_b64_appended(arrays)
    ...
    for name, off in zip(fields.keys(), offsets):
        lines.append(
...
def _write_vtr_companion(path: Path,
    ...
    field_arrays = [arr.transpose(2, 1, 0).ravel(order="C")
                    for arr in fields.values()]
    coord_arrays = [np.ascontiguousarray(x, dtype=np.float64),
                    np.ascontiguousarray(y, dtype=np.float64),
                    np.ascontiguousarray(z, dtype=np.float64)]
    payload_b64, offsets = _encode_b64_appended(field_arrays
                                                 + coord_arrays)
    ...
    for name, off in zip(fields.keys(), offsets[:n_fields]):
        lines.append(
            f'        <DataArray type="Float64" Name="{name}" '
            f'format="appended" offset="{off}"/>')
    lines.extend([
        '      </PointData>',
        '      <Coordinates>',
        f'        <DataArray type="Float64" Name="X" '
        f'format="appended" offset="{offsets[n_fields]}"/>',
        f'        <DataArray type="Float64" Name="Y" '
        f'format="appended" offset="{offsets[n_fields + 1]}"/>',
        f'        <DataArray type="Float64" Name="Z" '
        f'format="appended" offset="{offsets[n_fields + 2]}"/>',
        '      </Coordinates>',
        '    </Piece>',
        '  </RectilinearGrid>',
        '  <AppendedData encoding="base64">',
...
def build_velocity_sidecar(raw_dir: Path, out_path: Path, grid_dx: float,
    ...
    # G-2: per-field sanity bounds.
    field_bounds = {
        "Vp":      (vp_min_mps, vp_max_mps, "m/s"),
        "Vs":      (vs_min_mps, vs_max_mps, "m/s"),
        "density": (rho_min_kgm3, rho_max_kgm3, "kg/m^3"),
    }
    for name in fields_xyz:
        if name not in field_bounds:
            # Map CVM-H column names to canonical names if needed.
            # Currently the reader returns lowercase column names; the
            # CVM-H archive uses 'vp', 'vs', 'density'. Translate to
    ...
    if extras:
        raise ValueError(
            ...
            f"canonical map and field_bounds to support them.")
    fields_canonical = {canonical[k]: v for k, v in fields_xyz.items()}
    if set(fields_canonical) != set(field_bounds):
        raise ValueError(
            f"build_velocity_cvmh: expected fields {sorted(field_bounds)}; "
            f"got {sorted(fields_canonical)}")

    for name, arr in fields_canonical.items():
        v_min, v_max, _ = field_bounds[name]
        below = arr < v_min
        above = arr > v_max
        if below.any() or above.any():
            arr_min = float(arr.min())
            arr_max = float(arr.max())
            # Find one offending cell index for the message.
            if below.any():
                i, j, k = np.unravel_index(int(np.argmin(arr)), arr.shape)
            else:
                i, j, k = np.unravel_index(int(np.argmax(arr)), arr.shape)
            raise ValueError(
                f"build_velocity_cvmh: field '{name}' has values out of "
                f"declared range [{v_min}, {v_max}].\n"
                f"  observed: min={arr_min:.3f}, max={arr_max:.3f}\n"
                f"  offending cell index (i,j,k)=({i},{j},{k}); "
                f"value={float(arr[i,j,k]):.3f}\n"
                f"  Likely cause: source data includes sub-seafloor / "
                f"water-saturated cells. Regenerate the input slices "
                f"to exclude those cells, or override the threshold "
                f"explicitly via --{name.lower()}-min-mps / "
                f"--{name.lower()}-max-mps.")
