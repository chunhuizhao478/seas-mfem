...
class Vs:
    ...
    def pool(self, Pc):
        """Z-only pool at the barycentre's (x, y) -- the original 1 Hz p5 rule."""
        bary = Pc.mean(1)
        v = self.at(bary)
        q = bary.copy()
        for k in range(4):
...
def failing(P, T, vs, gate, chunk=CH, pooled=False):
    ...
    for s0 in range(0, len(T), chunk):
        ...
        dx = E.max(1)
        bary = Pc.mean(1)
        v = vs.pool(Pc) if pooled else vs.at(bary)
...
def main():
    ...
    ap.add_argument("--vs-source", choices=["deck", "muscal"], default="muscal",
                    help="velocity model the gate and the target read. 'deck' is "
                         "the run-time nc (1500 m lateral, 250 m UNIFORM vertical); "
                         "'muscal' is the source model it was resampled from "
                         "(0.01 deg, 50 m depth step in the top 500 m). Nearest-grid "
                         "on the deck file hands every barycentre in the top 125 m "
                         "the z=0 value, where MUSCAL resolves rock 2.2x faster.")
    args = ap.parse_args()
    t0 = time.time()

    if args.vs_source == "muscal":
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        ...
        class _M(MuscalVs):
            def pool(self, Pc):
                ...
                descendant, provably convergent, and what the deck nc needed --
                there, Vs steps 3.78x across the z=-125 m bin edge, so one
                bisection could drop a child's Vs 4x and the loop treadmilled.

                MUSCAL has no such cliff: its depth ladder is 50 m through the
                top 500 m, so Vs varies smoothly and one bisection moves it a
                little.  Measured on the size field, gradient-limiting the
                MUSCAL-feasible ceiling costs +10 % against +97 % on the deck nc
                -- i.e. the forbidden zone all but disappears.  frac=0.25 then
                suffices: it bounds the NEXT generation (a child's barycentre
                lies within about half the parent's extent) and is re-evaluated
                every round, at a fraction of the refinement.  The full bound
                flagged 1,456,239 cells against 39,281 that actually fail.
                """
                if frac <= 0.0:
                    return self.at(Pc.mean(1))
                b = Pc.mean(1)
                r = frac * (Pc.max(1) - Pc.min(1))
                return self.bbox_min(b - r, b + r)

        vs = _M(backup=Vs(args.cvm))
    else:
        vs = Vs(args.cvm)
