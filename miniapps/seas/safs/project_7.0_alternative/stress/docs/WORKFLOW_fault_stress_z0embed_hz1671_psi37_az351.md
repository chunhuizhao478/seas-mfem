...
| **`hz1671`** | **H**ickman & **Z**oback (2004) SAFOD magnitudes at **1671 m** depth: `SHmax=113, Shmin=49, Sv=45, P_p=16` MPa | `hickman_and_zoback…py:328,388` |
| **`psi37`** | **Ψ ≈ 37°** — angle between SHmax and the SAF strike (`\|351 − 314\| = 37`). The *intermediate/shallow* value from H&Z's depth table, vs the deep Ψ = 69° used in `demo_safod()` | `hickman_and_zoback…py:379` |
...
meshing/results/vtu/…_z0embed_fault.vtu  ─┐  (triangulated SAF surface)
   ...
   SHmax_az=351  fault_strike_hint=314    │
           ...
           project_to_fault_stress.py  (single-file mode)
                                          │
   ① build_bulk_stress_tensor(σ⁰)   3×3 Cauchy in (E,N,Up), compression POSITIVE
   ② triangle_geometry              per-tri centroid + unit normal
   ③ build_fault_basis              Tandem (s = up×n̂, d = s×n̂, n̂), strike→314° hint
   ④ resolve_traction(σ⁰,n̂,s,d,P_p) σ_n, σ_n_eff, τ_strike, τ_dip, |τ|, rake, μ_app
...
The projector consumes the **ParaView VTU split** of the volume `.msh`, produced
by `meshing/code/msh_to_vtu.py` (PIPELINE Stage 7):

| File | Role |
|---|---|
| `meshing/results/vtu/safs_fault_box_nwcut_500m_lcfar3000_z0embed_fault.vtu` | the 42 358-triangle SAF surface (Physical Surface 101) — **arg 1** |
| `meshing/results/vtu/safs_fault_box_nwcut_500m_lcfar3000_z0embed_bulk.vtu`  | the bulk tetra mesh — auto-found by `_derive_bulk_path()` for `--write-bulk` |
