...
The "slice" is built by, for every tet that straddles z=-4000 m,
sampling the FE-linear value at the tet's xy-centroid lifted to
z=-4000 m.  Linear barycentric on the 4 corner DOFs.
...
PREVIEW_OLD = (f"{ROOT}/velocity/results/cvmh/preview/"
               "projected_velocity_500m/Cycle000000/proc000000.vtu")
PREVIEW_NEW = (f"{ROOT}/velocity/results/cvmh/preview/"
               "projected_velocity_500m_d80km/Cycle000000/proc000000.vtu")
PREVIEW_NEW2 = (f"{ROOT}/velocity/results/cvmh/preview/"
                "projected_velocity_500m_lcfar3000/Cycle000000/proc000000.vtu")
PREVIEW_NEW3 = (f"{ROOT}/velocity/results/cvmh/preview/"
...
OUT = f"{ROOT}/velocity/results/cvmh/comparison_old_vs_new_baseline_z4km.png"
...
def print_residual_table(name, xc, yc, gf, truth, z_height, trace):
    ...
    diff = gf - truth
    print(f"\n=== {name}  ({gf.size} tets straddle z=-4000 m) ===")
...
def main():
    ...
    oxc, oyc, ogf, otrue, on_c, oz_h = tet_slice(
        PREVIEW_OLD, gx_s, gy_s, gz_s, Vs, z_target=z_target)
    print(f"  OLD: {on_c} cells, {ogf.size} straddle z=-4000 m")
    ...
    nxc, nyc, ngf, ntrue, nn_c, nz_h = tet_slice(
        PREVIEW_NEW, gx_s, gy_s, gz_s, Vs, z_target=z_target)
    print(f"  NEW: {nn_c} cells, {ngf.size} straddle z=-4000 m")

    has_new2 = os.path.isfile(PREVIEW_NEW2)
    if has_new2:
        print(f"reading NEW2 vtu: {PREVIEW_NEW2}")
        n2xc, n2yc, n2gf, n2true, n2_c, n2z_h = tet_slice(
            PREVIEW_NEW2, gx_s, gy_s, gz_s, Vs, z_target=z_target)
        print(f"  NEW2: {n2_c} cells, {n2gf.size} straddle z=-4000 m")
    else:
    ...
    s1 = ax1.scatter(o_lon, o_lat, s=2, c=ogf / 1000.0,
                     cmap=cmap_v, vmin=vmin, vmax=vmax,
                     edgecolors="none")
    setup(ax1, f"OLD baseline H1-P1 slice @ z=-4000 m\n"
    ...
    s2 = ax2.scatter(n_lon, n_lat, s=2, c=ngf / 1000.0,
                     cmap=cmap_v, vmin=vmin, vmax=vmax,
                     edgecolors="none")
    setup(ax2, f"NEW baseline H1-P1 slice @ z=-4000 m\n"
    ...
    if has_new2:
        ...
        s3 = ax3.scatter(n2_lon, n2_lat, s=2, c=n2gf / 1000.0,
                         cmap=cmap_v, vmin=vmin, vmax=vmax,
                         edgecolors="none")
        setup(ax3, f"NEW2 baseline H1-P1 slice @ z=-4000 m\n"
