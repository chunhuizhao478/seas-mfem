...
Checks:
  ...
  F4a free-surface total AREA unchanged (TOPOGRAPHY-AWARE: works for a flat lid
      AND a topographic one -- refining a planar triangle preserves its area)
  F4b free-surface z-range unchanged
  F5  0 inverted tets
"""
import argparse
import sys
...
def top_stats(P, C, cdx):
    ...
    the same statement, so one check serves both lineages.)
    """
    tv, ar = [], 0.0
    for s in range(4):
        sel = np.nonzero(cdx[:, s] == 1)[0]
        if not sel.size:
            continue
        tri = C[sel][:, LOCAL_FACES[s]]
        tv.append(tri.ravel())
        q = P[tri]
        ar += 0.5 * np.linalg.norm(
            np.cross(q[:, 1] - q[:, 0], q[:, 2] - q[:, 0]), axis=1).sum()
    v = np.unique(np.concatenate(tv))
    return v, float(ar), float(P[v, 2].min()), float(P[v, 2].max())
...
def main():
    ...
    ap_area = area(Tp)
    _, ar_p, zlo_p, zhi_p = top_stats(Pp, Cp, codes_of(Bp))   # BEFORE freeing the parent
    del Pp, Cp, Bp
    Pn, Cn, Bn = load(a.new)
    ...
    topv, ar_n, zlo_n, zhi_n = top_stats(Pn, Cn, cd)
    flat = max(abs(zlo_p), abs(zhi_p)) < 1e-6
    chk("F4a free-surface area unchanged", abs(ar_p - ar_n) / max(ar_p, 1) < 1e-12,
