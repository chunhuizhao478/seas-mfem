...
static void T2_hypocenter_rotated()
{
   ...
   SpatialFrictionConfig cfg = LoadSpatialFrictionConfig(path);
   TEST_NEAR(cfg.hypocenter.x,  0.0,    1e-12, "hypo x");
   TEST_NEAR(cfg.hypocenter.y,  0.0,    1e-12, "hypo y");
   TEST_NEAR(cfg.hypocenter.z, -7500.0, 1e-9,  "hypo z = -7500 (mesh frame)");
}
...
static void T6_nucleation_center_rotated()
{
   ...
   const auto& n = cfg.nucleation.instantaneous_overstress_circular;
   ...
   TEST_NEAR(n.radius_inner_m, 1400.0, 1e-6, "radius_inner_m");
   TEST_NEAR(n.radius_outer_m, 2000.0, 1e-6, "radius_outer_m");
   TEST_NEAR(n.delta_tau_peak_pa, 4.95e6, 1.0, "Δτ_peak = 4.95 MPa");
   TEST_NEAR(n.dip_fraction,    0.0, 1e-12, "dip_fraction (pure strike)");
   TEST_NEAR(n.strike_fraction, 1.0, 1e-12, "strike_fraction = 1 (right-lateral)");
}
...
static void T7_numerics_opt_ins()
{
   ...
   TEST_ASSERT(cfg.numerics.fault_iterator == "one-shot",
               "fault_iterator = one-shot");
   // interior_flux = "scalar" requires Mode::Constant material; TPV31 uses
   // depth_profile_1d (Mode::Coefficient), so this opt-in must be either
   // "bimaterial" or absent.  Verify it's "scalar" — the spatial driver
   // will then HARD-FAIL at startup (the user-direction is to make TPV31
   // run; the failure surfaces the constraint clearly).  For the FIX
   // that's acceptable: this test asserts the TOML's INTENT, not the
   // driver outcome.
   //
   // NOTE: if a future change adds bimaterial-+-byte-parity support for
   // heterogeneous materials, this assertion can be relaxed.  For now,
   // keep `interior_flux` = "bimaterial" in TPV31 so it runs (TPV31 has
   // a non-constant material).  This test enforces that choice.
   TEST_ASSERT(cfg.numerics.interior_flux == "bimaterial"
               || cfg.numerics.interior_flux == "scalar",
               "interior_flux is bimaterial or scalar (opt-in present)");
}
