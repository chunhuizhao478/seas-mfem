                                    ...
                                    real_t V_vec[2])
{
   ...
   // v55: anti-parallel to tau (Tandem convention)
...
void TestBP5ComputeRHS()
{
   ...
   for (int i = 0; i < N; i++)
   {
      ...
      // v55: V should be ANTI-parallel to tau_pre (Tandem convention) → dot < 0
      if (dot > 1e-20)
      {
         direction_ok = false;
         break;
...
void TestBP5BelowFault()
{
   ...
   for (int i = 0; i < N; i++)
   {
      ...
      {
         below_count++;
         // v55: Below-fault DOFs should have rate = (0, -Vp, 0) (Tandem convention)
