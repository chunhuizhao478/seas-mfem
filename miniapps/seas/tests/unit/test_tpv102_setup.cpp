...
void TestBoxcarFunction()
{
   ...
   // Well inside: B(10km, 15km, 3km) = 1 (10 < 15-3 = 12)
   TEST_NEAR(Boxcar(10e3, W, w), 1.0, 1e-12, "B(10km) = 1 (inside)");

   // At inner edge: B(12km, 15km, 3km) = 1 (exactly at W-w)
   TEST_NEAR(Boxcar(12e3, W, w), 1.0, 1e-12, "B(12km) = 1 (inner edge)");

   // In transition: B(15km, 15km, 3km) = 0.5 (midpoint)
   TEST_NEAR(Boxcar(15e3, W, w), 0.5, 0.01, "B(15km) ≈ 0.5 (midpoint)");
...
void TestSpatiallyVaryingA()
{
   ...
   // At hypocenter (0, -7.5 km): inside VW zone → a = a_vw = 0.008
   ...
   // Transition check: a should be between a_vw and a_vs
   real_t a_trans = ComputeA(0.0, 15e3);  // at VW edge along dip
   TEST_ASSERT(a_trans > TPV102Params::a_vw && a_trans < TPV102Params::a_vs,
               "a(0, -15km) in transition (" + std::to_string(a_trans) + ")");
}
