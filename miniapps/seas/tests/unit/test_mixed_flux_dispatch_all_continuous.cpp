// Round-13 R-1402 — end-to-end AllContinuous-mode dispatch correctness.
//
...
//
//   Gate 1: Q_new_all differs from Q_new_none by a non-trivial amount
//           (the central dispatch is actually firing).  Quantitatively,
//           max |Q_new_all − Q_new_none| > scale_dependent_floor.
//
...
//           never-touched-AllContinuous run (idempotency / clean-up
//           regression — guards against latent state from the
//           AllContinuous build that survives the None reset).
//
// Fixture: BuildSmallFaultTetMesh (48 tets — 8 hexes × 6-tet split;
// fault at y=L/2; |central_flux_face_set_| in AllContinuous mode is
// 64 — DOUBLE the Adjacent count of 32 on the same fixture).  Larger
// central set amplifies any sign/magnitude bug in Central by ~2× vs
// the Adjacent test, providing complementary coverage of Zhang's
// Mixed-Flux 1 variant exposed via `--mixed-flux all-continuous`.
...
// R-1200: assemble the analytic prediction of dQdt_all − dQdt_none
...
//
// The result must equal Mult(Q_init, mode=AllContinuous) − Mult(Q_init, mode=None)
...
//
// R-1504: the helper is named "Adjacent" for historical parity with
// `test_mixed_flux_dispatch_adjacent.cpp` but is MODE-NEUTRAL — it
// iterates over whatever set is in `wave.GetCentralFluxFaceSet()`,
// so it works for both Adjacent and AllContinuous (the latter just
// has a larger set).
...
int main()
{
   std::cout << "\n=== Round-13 R-1402: AllContinuous-mode dispatch correctness ===\n";
   ...
   // ----------------------------------------------------------------
   // Path B: mode AllContinuous on a fresh wave.  Q_new_all.
   // ----------------------------------------------------------------
   ...
   wave_b.SetMixedFluxMode(MixedFluxMode::AllContinuous);
   const std::size_t central_set_size = wave_b.GetCentralFluxFaceSet().size();
   std::cout << "  |central_flux_face_set_| (AllContinuous) = "
             << central_set_size << "\n";

   Vector Q_new_all(NUM_STATE * ndof_total);
   RunOneStepGivenIC(wave_b, dof_b, mesh_b, order, Q_init, Q_new_all);

   const real_t diff_adj_vs_none = MaxAbsDiff(Q_new_all, Q_new_none);
   std::cout << "  max |Q_new_all − Q_new_none| = " << std::scientific
   ...
   TEST_GE(diff_adj_vs_none / scale_q_new, 1e-9,
           "Gate 1: Q_new_all differs from Q_new_none — AllContinuous dispatch fires");
   ...
   TEST_LE(diff_adj_vs_none / scale_q_new, 1.0,
           "Gate 2: Q_new_all − Q_new_none is bounded (no blowup)");
   ...
   Mesh mesh_d_all = BuildSmallFaultTetMesh(L);
   WaveOperator<Mesh> wave_d_all(mesh_d_all, order,
                                  TPV102Params::lambda, TPV102Params::mu,
                                  TPV102Params::rho, bc);
   FaultFaceFlux ff_d_all(TPV102Params::rho, TPV102Params::cp,
                          TPV102Params::cs);
   wave_d_all.SetFaultFlux(&ff_d_all);
   std::vector<DOFData> dof_d_all;
   SetupWaveForRun(wave_d_all, dof_d_all, mesh_d_all, order);
   wave_d_all.SetMixedFluxMode(MixedFluxMode::AllContinuous);
   Vector dQdt_all(NUM_STATE * ndof_total);
   wave_d_all.Mult(Q_init, dQdt_all);

   Vector observed_delta(NUM_STATE * ndof_total);
   for (int i = 0; i < observed_delta.Size(); i++)
   {
      observed_delta(i) = dQdt_all(i) - dQdt_none(i);
   }
   const real_t observed_scale = MaxAbs(observed_delta);
   std::cout << "  max |dQdt_all − dQdt_none| (Mult/RK1) = "
             << std::scientific << std::setprecision(3)
             << observed_scale << "\n";

   Vector expected_delta(NUM_STATE * ndof_total);
   ComputeAnalyticAdjacentDelta(wave_d_all, mesh_d_all, order, Q_init,
                                expected_delta);
   ...
   TEST_LE(analytic_rel, 1.0e-12,
           "Gate 2-analytic (R-1200): dQdt_all − dQdt_none equals the "
   ...
   // ----------------------------------------------------------------
   // R-1501: Gate 2-analytic-ADER for AllContinuous mode.  Mirrors the
   // analogous gate in test_mixed_flux_dispatch_adjacent.cpp; verifies
   // the Phase 4 algebraic identity through the AdvanceADER production
   // code path (sites at wave_operator.inl:3993 / :4478) on the LARGER
   // central set (~64 faces vs ~32 for Adjacent).  AllContinuous
   // amplifies any sign/magnitude bug ~2× vs Adjacent.
   // ----------------------------------------------------------------
   {
      // R-1604: construct FRESH WaveOperator instances rather than
      // reusing wave_d_none / wave_d_all which already had Mult called.
      // See R-1604 commentary in test_mixed_flux_dispatch_adjacent.cpp
      // for the test-isolation rationale.
      ...
      Mesh mesh_e_all = BuildSmallFaultTetMesh(L);
      WaveOperator<Mesh> wave_e_all(mesh_e_all, order,
                                     TPV102Params::lambda, TPV102Params::mu,
                                     TPV102Params::rho, bc);
      FaultFaceFlux ff_e_all(TPV102Params::rho, TPV102Params::cp,
                              TPV102Params::cs);
      wave_e_all.SetFaultFlux(&ff_e_all);
      std::vector<DOFData> dof_e_all;
      SetupWaveForRun(wave_e_all, dof_e_all, mesh_e_all, order);
      wave_e_all.SetMixedFluxMode(MixedFluxMode::AllContinuous);

      const real_t dt_ader = 1.0e-5;
      Vector Q_new_none_ader(NUM_STATE * ndof_total);
      Vector Q_new_all_ader(NUM_STATE * ndof_total);
      wave_e_none.AdvanceADER(Q_init, dt_ader, /*ader_order=*/2, Q_new_none_ader);
      wave_e_all .AdvanceADER(Q_init, dt_ader, /*ader_order=*/2, Q_new_all_ader);

      Vector observed_delta_ader(NUM_STATE * ndof_total);
      for (int i = 0; i < observed_delta_ader.Size(); i++)
      {
         observed_delta_ader(i) = Q_new_all_ader(i) - Q_new_none_ader(i);
      }
      const real_t scale_ader = MaxAbs(observed_delta_ader);

      Vector I(NUM_STATE * ndof_total);
      wave_e_all.ComputeADERTimeIntegrated(Q_init, dt_ader, 2, I);
      Vector expected_delta_ader(NUM_STATE * ndof_total);
      ComputeAnalyticAdjacentDelta(wave_e_all, mesh_e_all, order, I,
                                   expected_delta_ader);
      ...
      std::cout << "  max |Q_new_all − Q_new_none|       (AdvanceADER) = "
      ...
      // gate above.  AllContinuous has a 2× larger central set than
      // Adjacent so FP accumulation is slightly more (more terms in
      // the analytic Σ), but empirical residual is still well inside
      // 1e-12.  Document deviation if a future fixture pushes the
      // residual above 1e-12.
      TEST_LE(rel_ader, 1.0e-12,
              "Gate 2-analytic-ADER (R-1501 + R-1603, AllContinuous): "
              "Q_new_all − Q_new_none equals "
              "Σ_f M⁻¹·w·shape·(F_int(I)−F_central(I)) at 1e-12 "
              "relative through the AdvanceADER (production) path "
              "with the larger central set");
      TEST_GE(scale_ader, 1.0,
              "Gate 2-analytic-ADER-floor: AdvanceADER central dispatch "
              "fires under AllContinuous — observed delta is non-trivial");
   }

   // ----------------------------------------------------------------
   // Gate 3: idempotency under AllContinuous → None transition on the
   // SAME wave operator (R-1103-companion).  Latent state from the
   // AllContinuous build (e.g., a flag, a stale set) must NOT survive the
   // mode reset.
   // ----------------------------------------------------------------
   ...
   wave_c.SetMixedFluxMode(MixedFluxMode::AllContinuous);
   wave_c.SetMixedFluxMode(MixedFluxMode::None);
   TEST_LE(static_cast<double>(wave_c.GetCentralFluxFaceSet().size()), 0.0,
           "Gate 3a: SetMixedFluxMode(None) clears the set after AllContinuous");
   ...
   std::cout << "  max |Q_new (AllContinuous->None) − Q_new_none| = "
             << std::scientific << std::setprecision(3) << diff_round_trip << "\n";
   TEST_LE(diff_round_trip, 0.0,
           "Gate 3b: Q_new bit-identical after AllContinuous->None transition");
