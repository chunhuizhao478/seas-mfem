// LTS Track-A A1: BITWISE ON-vs-OFF gate for the per-tick exchange merge.
//
// Runs the SAME multi-cluster, diff-1, cross-rank sync sequence twice -- once on
// the per-correct exchange path (--lts-merge-exchanges OFF) and once on the merged
// path (ON) -- and requires the resulting state to be IDENTICAL TO THE BYTE.  This
// is the gate that turns the design's bitwise-identity ARGUMENT
// (document/comm_dev/DESIGN_a1_per_tick_merge_2026-07-24.md) into a RESULT.
//
// The banded fixture (inherited from test_lts_mpi_seam_diff1) is what makes this
// meaningful: at np==NClust every band boundary is a RANK-SEAM diff-1 face, so both
// merged rounds are exercised -- the batched I exchange AND the premultiplied
// coarse-forecast exchange -- and a middle band is provider and consumer at once.
//
// Run: mpirun -np 2 ./seas_test_lts_a1_merge_np2
//      SEAM_DIFF1_NC=3 mpirun -np 3 ./seas_test_lts_a1_merge_np2   (3-cluster chain)
...
int main(int argc, char *argv[])
{
                           ...
                           long long &n_sched) -> void
   {
      ...
      LtsBulkSyncStepper<ParMesh> st(wave, layout, cluster_id, Q_out, ader_order,
                                     dt_base);
   ...
   if (nprocs > 1)
   {
      ...
      if (rank == 0)
      {
         std::printf("A1 merge: collectives %lld -> %lld over %d syncs "
   ...
   if (rank == 0)
   {
      std::printf("test_lts_a1_merge_np2 (np=%d): %d/%d passed, %d failed.\n",
