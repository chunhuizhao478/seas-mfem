...
// ---------------------------------------------------------------------------
// AdvanceWithSubStepStates — callback-aware variant (Phase N analog).
// ---------------------------------------------------------------------------
void Tpv104SubStepIterator::AdvanceWithSubStepStates(
   std::vector<DOFData> &dof_data,
   const std::vector<Vector> &fault_coords,
   ...
   FrictionSolver::Method method)
{
   ...
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): SetSubSteps "
         "has not been called");
   }
   if (!std::isfinite(dt_macro) || dt_macro <= 0.0)
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): dt_macro <= 0");
   }
   if (!std::isfinite(t_macro_start))
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): t_macro_start "
         "not finite");
   }
   if (I_imp_plus_flat == nullptr || I_imp_minus_flat == nullptr)
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): I_imp_*_flat "
         "must both be non-null.");
   }
   ...
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): "
       ...
       || static_cast<int>(V_w.size()) != n)
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): "
         "dof_data / fault_coords / V_w size mismatch");
   }
   ...
   {
      throw std::runtime_error(
         "Tpv104SubStepIterator::AdvanceWithSubStepStates(cb): Σ deltaT != "
         "dt_macro");
   }
