...
build_caliper() {
    ...
    if ! (
        ...
        # (timer PC sampling -> which function is slowest, incl. uninstrumented
        # PETSc/MUMPS/HDF5/MPI) is dependency-free; WITH_PAPI (hardware counters
        # -> the "why": IPC, L2/L3 misses) is added ONLY if PAPI is found, so a
        # missing PAPI never breaks the (non-fatal) Caliper build.  Frontera is
        # Intel, so PAPI also supports Intel 'topdown'; callpath auto-detects
        # libunwind for sample call-trees.
        _cali_prof=( -DWITH_SAMPLER=ON )
        module load papi 2>/dev/null || true
        _papi_root="${TACC_PAPI_DIR:-${PAPI_HOME:-${PAPI_ROOT:-${PAPI_DIR:-}}}}"
